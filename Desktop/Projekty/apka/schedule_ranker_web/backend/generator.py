...
# Dodajemy katalog backend do sys.path, aby importować moduły wewnątrz niego
sys.path.append(str(Path(__file__).parent))

from db import DB_PATH
from candidate_service import save_candidate
from fet_ltr import build_synthetic_candidates
...
def generate_and_save_synthetic_candidates(n_candidates: int = 30, random_state: int = 42):
    ...
    for _, row in df.iterrows():
        ...
        print(f"✅ Zapisano {candidate['id']} (profil: {candidate['profile']})")
